...
def main() -> int:
    ...
    storage = Path(
        __import__(
            "tradedesk.backend.config", fromlist=["settings"]
        ).settings.diagnostics_storage_dir
    )
    if args.dry_run:
        ...
        for p in storage.iterdir():
            try:
                if (
                    p.is_file()
                    and datetime.utcfromtimestamp(p.stat().st_mtime) < cutoff
                ):
                    print(p)
            except Exception:
                continue
        nonces_dir = Path(
            __import__(
                "tradedesk.backend.config", fromlist=["settings"]
            ).settings.diagnostics_nonces_dir
            or (storage / "nonces")
        )
        nonce_cutoff = datetime.now(timezone.utc) - timedelta(
            days=__import__(
                "tradedesk.backend.config", fromlist=["settings"]
            ).settings.diagnostics_nonces_ttl_days
        )
        if nonces_dir.exists():
            for n in nonces_dir.iterdir():
                try:
                    if (
                        n.is_file()
                        and datetime.utcfromtimestamp(n.stat().st_mtime) < nonce_cutoff
                    ):
                        print(n)
                except Exception:
                    continue
