...
def send_sms(to_number: str, message: str) -> None:
    ...
    if provider == 'twilio':
        ...
        if not settings.sms_twilio_account_sid or not settings.sms_twilio_auth_token or not settings.sms_from_number:
            raise RuntimeError('Twilio credentials not configured')
        client = Client(settings.sms_twilio_account_sid, settings.sms_twilio_auth_token)
        client.messages.create(body=message, from_=settings.sms_from_number, to=to_number)
        return
