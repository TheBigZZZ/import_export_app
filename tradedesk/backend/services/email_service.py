...
def send_simple_email(to_email: str, subject: str, body: str) -> None:
    ...
    msg["From"] = (
        ...
        or settings.diagnostics_notify_email_from
    ...
    items = (
        await db.execute(
            (
                ...
                "FROM sales_invoice_items WHERE invoice_id = :iid"
            ),
            {"iid": invoice_id},
        )
    ).all()
...
def send_email_with_attachment(
    ...
    try:
        ...
        if settings.diagnostics_smtp_user and smtp_password:
            with smtplib.SMTP(host, port, timeout=10) as smtp:
                if (
                    settings.diagnostics_smtp_port == 587
                    and settings.diagnostics_smtp_user
                ):
                    smtp.starttls()
                smtp.login(settings.diagnostics_smtp_user, smtp_password)
                smtp.send_message(msg)
