...
from .accounting_service import AccountingValidationError, JournalLine, validate_double_entry
...
class VoucherService:
    ...
    async def _next_voucher_no(self, voucher_type: VoucherType, year: int) -> str:
        ...
        stmt = select(Transaction.voucher_no).where(Transaction.voucher_no.like(f"{prefix}%"))
    ...
    async def list_vouchers(self) -> list[VoucherRead]:
        rows = await self.db.execute(select(Transaction).order_by(Transaction.transaction_date.desc(), Transaction.id.desc()))
        ...
        vouchers.sort(key=lambda item: (item.transaction_date, item.voucher_no), reverse=True)
        return vouchers

    async def get_voucher_by_line_id(self, line_id: int) -> VoucherRead | None:
        result = await self.db.execute(select(Transaction).where(Transaction.id == line_id))
        line = result.scalar_one_or_none()
        if not line:
            return None

        rows = await self.db.execute(
            select(Transaction).where(Transaction.voucher_no == line.voucher_no).order_by(Transaction.id.asc())
        )
    ...
    async def create_voucher(self, payload: VoucherCreate, created_by: int | None = None) -> VoucherRead:
        try:
            voucher_type = VoucherType(payload.voucher_type)
        except ValueError as exc:
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Invalid voucher type") from exc

        validate_double_entry([JournalLine(debit=Decimal(line.debit), credit=Decimal(line.credit)) for line in payload.lines])

        account_ids = [line.account_id for line in payload.lines]
        accounts = await self.db.execute(select(ChartOfAccount.id).where(ChartOfAccount.id.in_(account_ids)))
        ...
        if missing:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Missing account(s): {missing}")

        voucher_no = await self._next_voucher_no(voucher_type, payload.transaction_date.year)
    ...
    async def update_voucher(self, line_id: int, payload: VoucherCreate, updated_by: int | None = None) -> VoucherRead:
        existing = await self.get_voucher_by_line_id(line_id)
        if not existing:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Voucher not found")

        rows = await self.db.execute(select(Transaction).where(Transaction.voucher_no == existing.voucher_no))
        for row in rows.scalars().all():
    ...
    async def void_voucher(self, line_id: int, reason: str, user_id: int | None = None) -> VoucherRead:
        existing = await self.get_voucher_by_line_id(line_id)
        if not existing:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Voucher not found")
        ...
        original_rows = await self.db.execute(
            select(Transaction).where(Transaction.voucher_no == existing.voucher_no).order_by(Transaction.id.asc())
        )
