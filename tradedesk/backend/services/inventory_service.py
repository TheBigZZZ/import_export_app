...
def apply_stock_movement(current_stock: Decimal, movement_qty: Decimal, allow_negative: bool = False) -> Decimal:
...
def validate_document_is_posted(document_status: str) -> None:
    if document_status.strip().lower() != "posted":
        raise StockValidationError("Inventory can be updated only when a document is posted")
