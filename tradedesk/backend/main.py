...
from .diagnostics import router as diagnostics_router, purge_old_diagnostics
...
try:
    setup_logging(log_dir=str(settings.logs_dir), level=logging.INFO if settings.environment == "production" else logging.DEBUG)
except Exception:
...
try:
    from .errors import http_exception_handler, generic_exception_handler
...
if not logging.getLogger().handlers:
    ...
    logging.getLogger().setLevel(logging.INFO if settings.environment == "production" else logging.DEBUG)
...
async def on_startup() -> None:
    # Ensure runtime directories exist (base/data/logs/backups)
    try:
    ...
    # Fresh-start reset option for development/testing. If TRADEDESK_RESET_ON_STARTUP
    # is set to a truthy value (1/true), remove the DB file and re-run migrations.
    try:
        ...
        if reset_flag in ("1", "true", "yes"):
            try:
                ...
                if dbp.exists():
                    logger.info("TRADEDESK_RESET_ON_STARTUP enabled — removing existing DB: %s", dbp)
                    try:
                        dbp.unlink()
                    except Exception:
    ...
    await seed_defaults()
    try:
        # Create diagnostics directories if configured
        try:
    ...
    # Start exchange-rate auto-sync background task if enabled
    try:
        if settings.exchange_rate_auto_sync:
            async def _rate_sync_loop():
                while True:
                    ...
                    await asyncio.sleep(settings.exchange_rate_sync_interval_minutes * 60)
...
async def on_shutdown() -> None:
    ...
    task = getattr(app.state, '_rate_sync_task', None)
    if task:
        task.cancel()
        try:
            await task
