...
async def dashboard_kpis(
	...
	_: object = Depends(require_roles("super_admin", "admin", "accounts_manager", "viewer", "sales_manager")),
) -> DashboardKpiResponse:
...
async def stock_position(
	db: AsyncSession = Depends(get_db),
	_: object = Depends(require_roles("super_admin", "admin", "accounts_manager", "viewer", "inventory_manager")),
) -> StockPositionResponse:
	return await ReportService(db).stock_position()


@router.get('/ar-aging', response_model=list[AgingResponse])
async def ar_aging(db: AsyncSession = Depends(get_db), _: object = Depends(require_roles("super_admin", "admin", "accounts_manager", "viewer"))):
	return await ReportService(db).ar_aging()


@router.get('/ap-aging', response_model=list[AgingResponse])
async def ap_aging(db: AsyncSession = Depends(get_db), _: object = Depends(require_roles("super_admin", "admin", "accounts_manager", "viewer"))):
