...
from ..schemas.party import (PartyLedgerResponse, SupplierCreate, SupplierRead,
                             SupplierUpdate)
...
async def list_suppliers(
    ...
    _: object = Depends(
        require_roles(
            "super_admin", "admin", "accounts_manager", "viewer", "purchase_manager"
        )
    ),
) -> list[SupplierRead]:
...
async def create_supplier(
    ...
    db: AsyncSession = Depends(get_db),
    _: object = Depends(
        require_roles("super_admin", "admin", "accounts_manager", "purchase_manager")
    ),
) -> SupplierRead:
