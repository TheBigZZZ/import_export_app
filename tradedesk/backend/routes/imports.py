...
from ..schemas.import_ops import ImportShipmentCreate, ImportShipmentRead, ImportPostResponse
...
async def list_import_shipments(
	...
	_: object = Depends(require_roles("super_admin", "admin", "purchase_manager", "inventory_manager", "accounts_manager", "viewer")),
...
async def create_import_shipment(
	...
	db: AsyncSession = Depends(get_db),
	user: User = Depends(require_roles("super_admin", "admin", "purchase_manager", "inventory_manager")),
) -> ImportShipmentRead:
...
async def post_import_shipment(
	shipment_id: int,
	db: AsyncSession = Depends(get_db),
	user: User = Depends(require_roles("super_admin", "admin", "purchase_manager", "inventory_manager", "accounts_manager")),
) -> ImportPostResponse:
