...
from ..schemas.voucher import VoucherCreate, VoucherListResponse, VoucherRead, VoucherVoidRequest
...
async def update_voucher(
	...
	try:
		return await VoucherService(db).update_voucher(line_id, payload, updated_by=user.id)
	except AccountingValidationError as exc:
		raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=str(exc)) from exc
...
async def void_voucher(
	...
	return await VoucherService(db).void_voucher(line_id, payload.reason, user_id=user.id)
