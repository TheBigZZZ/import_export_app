...
from PySide6.QtWidgets import QComboBox, QFileDialog, QHBoxLayout, QLabel, QMessageBox, QPushButton
...
class ReportsModule(BaseModuleWidget):
    ...
    def refresh(self) -> None:
        ...
        try:
            response = asyncio.run(self.api_client.get(endpoint))
        ...
        if title == "Trial Balance":
            ...
            self.current_headers = ["Account ID", "Code", "Name", "Debit", "Credit"]
            self.current_rows = [
                [
                    str(row["account_id"]),
                    row["account_code"],
            ...
            self.summary.setText(
                f"Total Debit: {payload.get('total_debit')} | Total Credit: {payload.get('total_credit')} | Balanced: {payload.get('is_balanced')}"
            )
        elif title == "Stock Position":
            rows = payload.get("rows", [])
            self.current_headers = ["Product ID", "Code", "Name", "Stock", "Unit Cost", "Value"]
            self.current_rows = [
                [
                    str(row["product_id"]),
                    row["product_code"],
            ...
            self.summary.setText(f"Total Stock Value: {payload.get('total_stock_value')}")
        else:
    ...
    def export_csv(self) -> None:
        ...
        file_path, _ = QFileDialog.getSaveFileName(self, "Export Report", str(Path.home()), "CSV Files (*.csv)")
        if not file_path:
            return
