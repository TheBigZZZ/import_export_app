...
def main():
    ...
    try:
        ...
        if not admin_user or not admin_pass:
            ...
            print(
                "No TRADEDESK_TEST_ADMIN_* env vars set; attempting to create temporary admin via CLI"
            )
            ...
            try:
                ...
                if res.returncode != 0:
                    print(
                        "CLI create-admin failed or users exist; continuing and attempting login anyway"
                    )
        ...
        status, _ = http_json(
            "POST", f"{BASE}/api/vouchers", voucher, headers=headers, timeout=20
        )
        if status >= 400:
